...
//   • _HeroHeader stat pills are now driven by real-time provider data:
//       – "Today's visitors" reads VisitorProvider.visitors filtered to today
//       – "Open complaints" reads ComplaintProvider.complaints filtered to OPEN
//       – "Notices" reads NoticeProvider.notices count
//   • NoticeProvider imported so the stat pill stays reactive
//   • ConnectivityWrapper in main.dart should also call
//     context.read<NoticeProvider>().syncPending() on reconnect (see main.dart)
//
// All other widgets (palette, _QuickActions, _ActivitySection, shared helpers)
// are kept exactly as-is.
...
import 'package:sms/providers/complaint_provider.dart';
import 'package:sms/providers/notice_provider.dart';
...
// ── Hero header — stat pills now read from live providers ─────────────────────
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // ── CHANGED: real-time stat pills ────────────────────────────
                  // Each pill is wrapped in its own Consumer so only that pill
                  // rebuilds when its provider changes.
                  Row(
                    children: [
                      // Today's visitors — count entries logged today
                      Consumer<VisitorProvider>(
                        builder: (_, vp, __) {
                          final today = DateTime.now();
                          final count = vp.visitors.where((v) {
                            final d = v.createdAt;
...
// ── Role-aware quick actions (unchanged) ──────────────────────────────────────
...
// ── Activity / Browse section (unchanged) ────────────────────────────────────
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              label,
              style: const TextStyle(color: Color(0xFF5A7A99), fontSize: 10),
              maxLines: 1,
